...
## The gallery, part one: reading lies
...
## The gallery, part two: writing blind
...
## Choosing a level: which anomalies your invariants depend on
...
- **5** — anomalies, named and priced (Dirty read, non-repeatable read, phantom, lost update, write skew — the catalog is short; the incidents are not.)
- **3** — phenomena in SQL-92 (Dirty / non-repeatable / phantom. The 1995 critique: ambiguous, and missing most of the zoo.)
- **1** — level stronger than its name (Postgres repeatable read is snapshot isolation: phantoms forbidden, write skew possible.)
- **0** — engines whose labels mean the standard (Oracle’s "serializable" is SI. Read the docs, not the adjective.)

---

**Q1. Transaction A reads count(*) WHERE on_call → 2 and clears its own on_call flag; transaction B does the same concurrently. Both commit; no doctor is on call. Under snapshot isolation, what just happened?**
   A. A phantom — B’s second read saw a different set
   B. A lost update — both wrote the same row
   C. Write skew — each txn read a state the other’s disjoint write invalidated; no shared row, so SI’s per-row mechanisms never fired
   D. A dirty read — one of them saw uncommitted data
   Answer: C — Reads overlap, writes are disjoint — the signature of write skew and the reason snapshot isolation is not serializable. It is also why this exact shape keeps shipping: each transaction is locally correct.

**Q2. Under Postgres REPEATABLE READ (snapshot isolation), which anomalies are prevented? (select all)**
   A. Dirty read
   B. Non-repeatable read
   C. Phantom
   D. Write skew
   Answer: A, B, C — The frozen snapshot forbids all three read-side anomalies — PG’s repeatable read is stronger than the standard’s name. Write skew survives, because the writes never collide on a row; that is what SERIALIZABLE (SSI, T4.L3) exists for.

**Q3. Your service enforces "at least one row with role='admin'" in application code: check the count, then delete or demote. Which level actually protects the invariant?**
   A. Read committed — the count re-reads fresh data each statement
   B. Repeatable read — the snapshot prevents phantoms
   C. Serializable (SSI) — the check-then-act pair is a write-skew shape only cycle detection aborts — or materialize the conflict with SELECT ... FOR UPDATE on a shared row
   D. Read uncommitted is fine — deletes are rare
   Answer: C — A cross-row invariant is exactly snapshot isolation’s blind spot. Either pay for serializable and retry the 40001s, or make the two transactions collide on purpose so first-writer-wins can see them.

---

The paper that broke the standard's table: **Berenson, Bernstein, Gray, Melton, O'Neil & O'Neil, "A Critique of ANSI SQL Isolation Levels" (SIGMOD 1995)** — short, polite, devastating; it names the phenomena the standard forgot and is the reason we talk about snapshot isolation at all. The modern map of the territory: **Jepsen's consistency models (jepsen.io/consistency)** — the zoo organized as a lattice, with the database-marketing translation layer included. The empirical answer to "what does my engine actually ship": **Martin Kleppmann's Hermitage (github.com/ept/hermitage)** — hand-run anomaly transcripts per engine and level; read your engine's file before your next migration. And the vendor truth, unusually candid: **PostgreSQL docs, "Transaction Isolation"** — one of the few manuals that admits which anomalies survive which level.
