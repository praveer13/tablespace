...
_Track T5: The Executor & the Planner · ~16 min · tablespace_

> The volcano model: open/next/close, pipelining, and why every executor you have ever used is the same six iterators. Then lab 05 — over your own pages and tree.
T1 through T4 built you a store: pages that account for every byte, a tree that stays balanced under fire, a log that survives its own death, versions that let readers and writers coexist. Impressive — and mute. It can *keep* data; it cannot answer a question. T5 gives it a mouth.
...
_fig 1 — a pull cascade: next() flows down, tuples flow up_

Components: Limit 10 (counts, then says done) · Project (keep name, email) · Filter (age > 30) · SeqScan (next LIVE slot) · IndexScan (swap-in leaf) · lab-01 pages (8192 B · 8 B header · 6 B slots) · lab-02 tree (descend by key)

Steps:
1. open() cascades root-to-leaf: each operator allocates its state; the leaf scan pins its first page and positions at slot 0.
2. The root gets its first next(): Limit asks Project, Project asks Filter, Filter asks SeqScan. One call stack, and not one tuple exists yet.
3. SeqScan reads the next LIVE slot off a lab-01 page — 8192-byte arena, 8-byte header, 6-byte slots — and returns the record. Nothing above it knows a page exists.
4. Filter checks age > 30: fails. The tuple dies here — Filter quietly pulls again, and its parent sees nothing of the miss.
5. A passing tuple goes up: Project trims it to (name, email); Limit counts 1. Pages touched so far: one.
6. Tenth tuple delivered: Limit returns done and never pulls an eleventh; close() unwinds top-down. A 10M-row table answered with ~10,000 leaf pulls.
7. Same tree, different leaf: swap SeqScan for an IndexScan over your lab-02 tree and every operator above runs unchanged — composition is the point.

---

## Where the pipeline must stop
...
**Q1. A plan is Limit(10) → Filter (selectivity ~1/1000) → SeqScan over a 10M-row table, no sort. Roughly how many tuples does SeqScan produce before the query finishes?**
   A. All 10M — the leaf always runs to completion
   B. ~10,000 — Limit stops pulling after its tenth tuple, so the cascade ends once Filter has passed ten rows
   C. ~1,000 — one per Filter rejection
   D. Exactly 10 — the leaf knows the LIMIT
   Answer: B — Pull is lazy all the way down. Limit never issues an eleventh next(); Filter keeps pulling until ten rows pass at ~1-in-1000 selectivity — about 10,000 leaf tuples, a handful of pages, done. This is why LIMIT with no sort above it is fast even on huge tables.

**Q2. Why must Sort buffer its entire input before returning its first tuple?**
   A. To build an index on the sort keys for binary search
   B. Sorting is CPU-bound, so batching amortizes cache misses
   C. The contract is global: the first tuple in sorted order might be the last tuple the child would ever produce — no honest tuple can ship before the input is exhausted
   D. The buffer pool requires full materialization before any operator output
   Answer: C — Global order needs global knowledge. That is what makes sort (and the hash join’s build side) a materialization point: the pipeline stops, buffers — in memory, or spilled to sequential disk runs — and only then resumes streaming.

**Q3. In the volcano model, how does a Filter operator drop a tuple that fails its predicate?**
   A. It pushes the tuple back to its child with a rejection flag
   B. It calls close() on the child and re-opens it at the next tuple
   C. It marks the tuple dead in its page slot so later scans skip it
   D. It simply pulls next() on its child again — the rejected tuple never travels upward, and the parent never learns it existed
   Answer: D — There is no push-back channel; the tree speaks one language, next(). Rejection is a private loop inside the operator — which is exactly why any operator can sit under any other without negotiation.

---

The source: **Graefe, "Volcano — An Extensible and Parallel Query Evaluation System" (IEEE TKDE, 1994)** — open/next/close, the exchange operator, and the argument that one interface buys extensibility and parallelism together. Its companion survey, **Graefe, "Query Evaluation Techniques for Large Databases" (ACM Computing Surveys, 1993)**, is the executor's whole toolbox — sorting, hashing, aggregation — in one paper; T5.L2 lives in its first chapters. The lectures: **CMU 15-445 (Pavlo), query execution** — the same model, live-coded. The production read: **Postgres's `src/backend/executor/README`** — ExecInitNode/ExecProcNode/ExecEndNode, shorter than you fear. And the honest counterpoint: **Neumann, "Efficiently Compiling Efficient Query Plans for Modern Hardware" (VLDB, 2011)** — push-based, compile-the-plan-to-code execution, the design every modern analytical engine (and CMU 15-721) prefers. The iterator you build in lab 05 optimizes for composition; Neumann optimizes for the CPU. Knowing why they differ is knowing the field. Now go write `executor.rs` — the suite is waiting.
