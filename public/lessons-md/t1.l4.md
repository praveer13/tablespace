# T1.L4 — Delete Is a Promise, Not an Erasure

_Track T1: Pages & Tuples · ~12 min · tablespace_

> Tombstones, free-space maps, and why "deleted" rows occupy disk until somebody does the accounting.
Run `DELETE FROM sessions WHERE expires_at < now()` and watch it remove forty million rows in seconds. Then check the table's file on disk: **not one byte smaller.** Nothing was erased, and nothing was supposed to be. DELETE in a real engine is not an erasure — it is a *promise*: a mark that says "this row is dead, reclaim it when it is provably safe." This lesson is about why the promise is necessary, who keeps the books in the meantime, and when the bytes actually come back. It closes the loop with lab 01, where you already implemented all three acts: the mark, the accounting, and the reclaim.

---

## Why erasure is not an option
...
## Tombstones: gone is not reclaimed
...
- **0 B** — erased at DELETE time (The header is marked (xmax / LIVE bit); the body stays until reclaim is provably safe.)
- **2 events** — gone vs reclaimed (Visibility is instant; space returns only after no snapshot can see the row and the bytes are made contiguous.)
- **32 B** — FSM bucket (Per-page free space, rounded down — one byte per page, max-aggregated up a tree.)
- **8184** — the storm’s final demand (Delete everything, one defrag: free_space() must equal a mint page. Not one leaked byte.)

---

## Back to lab 01: the grader counts
...
**Q1. Why can’t DELETE simply overwrite the row’s bytes and move on?**
   A. Because overwriting requires an exclusive lock on the page
   B. Because a transaction holding an older snapshot may still be entitled to read the row — and the delete itself may still roll back, so the bytes must survive until the outcome is durable
   C. Because the WAL must be checkpointed first
   D. It can — engines erase immediately; vacuum is only for index cleanup
   Answer: B — Two independent reasons: snapshot visibility (erasure would corrupt another transaction’s as-of reality — T4’s whole subject) and atomicity (an uncommitted delete must be undoable). So DELETE marks the header and the body stays until reclaim is provably safe.

**Q2. In lab 01, free_space() jumps the moment you delete a record, yet a 512 B insert is refused until defrag runs. What exactly is the distinction?**
   A. A bug — free_space() should only count allocatable bytes
   B. “Gone” vs “reclaimed”: dead_bytes count as free in the accounting immediately, but only defrag makes them contiguous and therefore allocatable to inserts
   C. The slot array is full, so no insert can proceed regardless of space
   D. The free-space map is stale and needs a refresh
   Answer: B — free_space() = contiguous + dead_bytes by design: it reports what defrag would reclaim. Insert fitness reads the contiguous region alone. The delete_reuse check exists to prove you kept those two numbers separate.

**Q3. The free-space map exists to…**
   A. Track which tuples are dead so vacuum can find them
   B. Record approximately how much free space each page has (rounded into 32-byte buckets, max-aggregated up a tree) so inserts find a candidate page without scanning the relation
   C. Map every ctid to its page for index lookups
   D. Remember which pages are pinned in the buffer pool
   Answer: B — It answers “which page has ≥ X bytes free?” in a tree descent instead of an O(relation) scan. Lossy is fine — a stale hint costs one wasted page visit, then the truth is recorded back. Dead-tuple tracking for vacuum is a different structure entirely.

---

The freespace machinery is documented in `src/backend/storage/freespace/README` (the bucket scheme and the max-aggregate tree), and the vacuum side is best learned from the interdb.jp *Internals of PostgreSQL* chapter 6 — the clearest free walkthrough of dead tuples, freeze, and what vacuum can and cannot reclaim — plus the PG docs chapter "Routine Vacuuming" for the operational knobs (`autovacuum` exists because "later" must actually arrive; a table that churns faster than its vacuum settings is the canonical bloat incident). Then look at your own tables: `pg_stat_user_tables` exposes `n_dead_tup` per table — the gone-but-not-reclaimed count, in the open, per relation. Next stop is T4 (`many-worlds-one-truth-each`), where `xmin`/`xmax` stop being trivia and become the visibility rule that decides exactly when "provably safe to reclaim" is true.
