# T2.L1 — The B+Tree Contract

_Track T2: Indexing · ~16 min · tablespace_

> Separators, fanout, and why every lookup costs exactly the same: the invariants that made this tree the default index for fifty years. Then you build one — lab 02.
...
## B+ vs B: records at the bottom, signposts above
...
```rust
// one level of the descent — tree.rs hands you this exact hint
...
_fig 1 — the anatomy: signposts above, records below, scans sideways_

Components: root — internal page ([ 50 ]) · internal page ([ 10 | 30 ]) · internal page ([ 70 ]) · leaf (3 · 7) · leaf (10 · 20) · leaf (30 · 44) · leaf (50 · 66) · leaf (70 · 88)

Steps:
1. The contract on one canvas: internal pages hold only separators — keys with child pointers, no records. Every record lives in a leaf; every leaf sits at the same depth; the leaves are linked left → right. That chain is the scan spine.
2. get(44): the root's only separator is 50, and 44 < 50 — descend left. On [10, 30], partition_point(separators ≤ 44) = 2, so the third child. Each level is one page and one binary search inside it.
3. get(50): 50 ≥ 50 — ties go right, because a separator is a copy of its right subtree's smallest key. Equality always descends to children[i+1].
4. scan(20, 70): descend once to the leaf that would hold 20, then walk the next chain until a key exceeds 70. One descent, then pure sideways motion — the internal pages are never touched again.
5. What you actually pay on disk: with ~500-way fanout the root and the next level are a few hundred pages — the buffer pool pins them. A point lookup is one or two real I/Os: the leaf, then the heap page the leaf entry points at.
...
**Q1. Why does a B+tree keep every record in the leaves instead of storing some in internal pages, the way the original B-tree does?**
   A. So that records stay sorted by primary key within each page
   B. So deletes never have to touch internal nodes
   C. Internal pages free of payload pack ~500 routing separators into 8KB — fanout goes up, the tree gets shallower, every lookup costs exactly height fetches, and the linked leaves make range scans a sideways walk
   D. Because records are too large to fit next to child pointers
   Answer: C — The plus is fanout plus scans. A separator is ~16 bytes; a record is ~100. Routing pages that hold only signposts make the tree three levels deep at 10M rows, and the leaf chain turns ranges into one descent plus a sideways walk. Uniform height also makes lookup cost a constant, not a distribution.

**Q2. An internal node holds separators [10, 30, 50]. A lookup for key 30 descends to which child?**
   A. children[1] — key 30 matches separator index 1
   B. children[2] — two separators are ≤ 30, and a separator is a copy of its right subtree's smallest key, so equality always descends right
   C. children[3] — the largest separator wins ties
   D. Either children[1] or children[2] — both subtrees may contain 30
   Answer: B — partition_point(|&s| s <= 30) over [10, 30, 50] is 2. The law keys[i] = min(children[i+1]) means a key equal to a separator lives in the subtree to that separator's right — never the left. Get the tie direction wrong and every exact-match lookup on a separator key silently misses.

**Q3. A table holds 10M rows of ~100 bytes (~80 per 8KB leaf), with ~500-way fanout. What does a point lookup cost, and why is the real number lower than the worst case?**
   A. ~24 page fetches — the depth of a balanced binary search
   B. 125,000 page fetches — one per leaf page, halved on average
   C. 3 tree fetches (80 → 40K → 20M covers 10M) plus 1 heap fetch; the root and next level are ~4MB and live in the buffer pool, so the disk usually sees only the leaf and the heap page
   D. 500 fetches — one per separator on the descent path
   Answer: C — Height is log base fanout: 3 levels at 10M rows. The tree's top is tiny and hot — the buffer pool pins it — so the quoted worst case (height + heap) and the common case (leaf + heap page, both from disk) differ by exactly the cached levels.

---

Read the people who own the format. Postgres's nbtree README (`src/backend/access/nbtree/README`) is the best written tour of a production B+tree — page layout, the high-key fence that makes concurrent descent safe (T4 will cash that check), and a deletion policy far lazier than any textbook admits. Alex Petrov's *Database Internals* (O'Reilly, 2019) chapter 2 is the cleanest taxonomy of the B-tree family, and the two B+tree lectures of **CMU 15-445** (Andy Pavlo, free on YouTube) walk the same descent you just wrote, at whiteboard speed. For lineage: Bayer & McCreight, "Organization and Maintenance of Large Ordered Indices" (1972) is the founding paper, and Douglas Comer's 1979 survey "The Ubiquitous B-Tree" named the era you still live in. Then open `tree.rs` — the storm is the teacher now.
