# TR.L4 — The Forge Workflow

_Track Tᴿ: Rust Zero · ~15 min · tablespace_

> rustup to a dropped .wasm: the forge loop — unzip, cargo test red→green, build for wasm32, watch the checks light up in the browser. Ends at lab 01.
The last piece of the ramp is not the language — it is the **loop**. Every forge lab runs the same five-step circuit: download, test, implement, build, drop. The Rust you just learned is the vocabulary; this lesson is the grammar of a working session, walked once end to end so that lab 01 costs you design effort and zero setup tax. Do the steps for real as you read — by the last section the toolchain will be warm and the only thing left will be the page.

---

## Step zero: the toolchain, once per machine
...
```bash
unzip slotted-pages.zip && cd slotted-pages
cargo test
...
- **1** — command in the inner loop (cargo test. Native, sub-second, same checks the browser runs.)
- **5** — checks grade lab 01 (insert_read · no_overlap · freespace_accounting · delete_reuse · storm.)
- **2000** — ops in the storm (Seeded, against a reference model — then a full-delete leak check that must end at exactly 8184 free.)
- **1** — file you may edit (src/page.rs in lab 01. The harness ships complete; your design fills one struct.)
...
**Q1. What is the forge loop, in order?**
   A. Build the wasm → drop it on the lab page → read the red checks → implement
   B. Unzip → cargo test (all red) → implement op by op until green → cargo build --release --target wasm32-unknown-unknown → drop the .wasm on the lab page
   C. Implement the whole file → cargo test once → fix everything → submit the .rs source
   D. Write tests against the harness API → implement → publish the crate
   Answer: B — Red first, on purpose: the traps are the checklist and the checks are the spec. Iterate natively with cargo test; the wasm drop is the grading step, not the development environment.

**Q2. Your lab is green locally but traps immediately in the browser. The template docs already told you the likely cause. What is it?**
   A. Release-mode optimizations reorder your writes; rebuild with --debug-assertions
   B. HashMap/HashSet draw OS randomness that does not exist on wasm32-unknown-unknown and panic there — switch to BTreeMap/BTreeSet
   C. The browser sandboxes float arithmetic; use integers only
   D. wasm has no heap; every allocation must be static
   Answer: B — std hash tables seed from the OS; the wasm32-unknown-unknown target has none to give. Ordered maps are deterministic on every target — which is also why the harness can replay your run exactly.

**Q3. Under a lab’s AGENTS.md, which agent behavior is inside the contract?**
   A. Writing the complete page.rs so you can study the finished style
   B. Refusing to discuss the lab at all, since the checks are graded
   C. Reading the failing check with you, naming the concept it tests, explaining the invariant, sketching a fragment at most — and stopping there
   D. Implementing only the hard methods and leaving the easy ones to you
   Answer: C — Tutor, never solver. The contract exists because the labs compound: lab 02 assumes you built lab 01. A solution you did not produce is a prerequisite you do not have.
