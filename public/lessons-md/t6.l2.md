# T6.L2 — HNSW, Layer by Layer

_Track T6: Vectors & HNSW · ~17 min · tablespace_

> Skip lists reborn as graphs: entry points, greedy descent, neighbor selection — the index behind pgvector, honestly derived. Then you build one — lab 06.
T6.L1 ended the exact dream and named the bargain: a little recall for orders of latency. This lesson builds the structure that keeps that bargain better than anything else anyone has fielded — **HNSW**, Hierarchical Navigable Small World, the index inside pgvector, and the index you are about to build.
...
## The layered trick: express lanes up top
...
_fig 1 — the layered descent: express lanes, then dense streets_

Components: entry (top of the graph) · node (dist 0.91 → 0.42) · layer 2 (express lanes) · same node (drop a layer) · node (dist 0.42 → 0.19) · layer 1 (mid-range hops) · layer 0 (dense streets · all nodes) · node (dist 0.19) · node (dist 0.09) · nearest (dist 0.02) · query (top-k returned)

Steps:
1. The query enters at the entry point — the single highest node — on the top layer. Almost the whole graph is invisible from here. That is the point.
2. Layer-2 greedy walk: hop to whichever neighbor is nearest the query. Few nodes, long edges — each hop cuts distance by a large factor: 0.91 → 0.42.
3. Local minimum: no layer-2 neighbor is closer. Do not fight it — drop to layer 1 from the node you stand on; it exists on every layer below its top.
4. Layer 1 resumes the walk with finer steps: 0.42 → 0.19, then another local minimum. Drop again.
5. Layer 0: dense streets, every node present. The walk now keeps a beam of ef_search candidates instead of a single position — the insurance against local minima.
6. The beam settles on the closest nodes found: 0.19 → 0.09 → 0.02. Return the best k. Total distance computations: a few hundred — against 5 million for the brute-force truth.

---

## Insert: descend, then connect carefully
...
> **[isomorphism]** You have met the layered trick twice already. The skip list is the obvious ancestor — same geometric promotion, same express-lane descent. But look at your lab-02 tree with this lesson's eyes: a B+tree is also "express levels over sorted data," and root-to-leaf descent is the same greedy walk with separators instead of distances — guaranteed exact, because one-dimensional order *can* be exact. HNSW is what that architecture becomes when T6.L1 takes exactness off the table: separators become edges, levels become layers, and the guarantee becomes a recall number.

---

- **~log n** — hops to anywhere (Layers scale the greedy walk: express lanes up top, dense streets at layer 0. Hundreds of distance computations per query, not millions.)
- **m = 16** — degree (2m at layer 0) (Edges per node, capped. Memory per node ≈ 2m edge ids plus the vector itself.)
- **ef_search** — the runtime dial (Beam width at query time: recall up, latency up, settable per query. Must be ≥ k.)
- **1.3 GB** — graph for 5M × m=16 (32 layer-0 edges × ~8B per id × 5M nodes. The vectors (~31GB) dwarf it.)

---

## Lab 06: you build this
...
**Browser lab: HNSW Explorer** — Step the greedy descent layer by layer and watch the ef beam decide your recall. (interactive, on the lesson page: https://tablespace.play.naigap.com/lesson/<this-lesson>)

---

**Q1. Mid-descent on layer ℓ, the greedy walk hits a local minimum — no neighbor is closer to the query than the current node. What happens next?**
   A. The search restarts from the entry point with a wider beam
   B. The walk backtracks to the previous node and tries its second-best neighbor
   C. The algorithm drops to layer ℓ−1 and resumes from the same node, which exists on every layer up to its top — the denser graph below takes over with finer steps
   D. The current node is returned as the approximate answer
   Answer: C — Layers are the escape hatch: a local minimum on a sparse express layer is just the signal to navigate more finely. The current node doubles as the entry point for the layer below — no restart, no backtracking.

**Q2. During insert, the neighbor-selection heuristic skips a candidate that is extremely close to the new node — because it is even closer to an already-selected neighbor. Why?**
   A. To keep edge lengths uniform for the cost model
   B. Degree is routing infrastructure: a second edge pointing the same direction as an existing one buys almost no reachability, and cliques of near-duplicates are factories for the local minima that stall greedy walks
   C. Duplicate edges violate the graph invariants the harness checks
   D. Closer candidates are numerically unstable in float arithmetic
   Answer: B — The m edges are a budget, spent on diverse directions. "Closest m" builds a trophy cabinet of similarity; the diversity heuristic builds a map. Reachability, not proximity — this rule is the difference between a small world and a hairball.

**Q3. Production recall is too low. A colleague proposes raising ef_construction from 64 to 256 on the live index. What happens?**
   A. Recall improves immediately — bigger beam, better candidates
   B. Build quality improves gradually as new rows arrive, and old edges heal over time
   C. Nothing — ef_construction is spent at build time; the existing graph's edges are already fixed, and only a rebuild can spend it. The live dial is ef_search
   D. The index rebuilds itself in the background
   Answer: C — The two efs are spent at different times: ef_construction buys edge quality once, at insert; ef_search buys candidate breadth per query. Raising the former without rebuilding is wishing on a graph that already exists; raising the latter trades latency for recall immediately — that trade is T6.L3’s curve.

---

The primary source: **Malkov & Yashunin, "Efficient and Robust Approximate Nearest Neighbor Search Using Hierarchical Navigable Small World Graphs" (IEEE TPAMI 2018; arXiv:1603.09320)** — Algorithms 1–5 are this lesson in pseudocode, including the diversity heuristic (their SELECT-NEIGHBORS-HEURISTIC) and the geometric level assignment. The ancestor: **Pugh, "Skip Lists: A Probabilistic Alternative to Balanced Trees" (1990)** — a few pages that make the layering feel inevitable. The theory underneath: **Kleinberg, "The Small-World Phenomenon: An Algorithmic Perspective" (STOC 2000)** — when greedy routing can work at all. The knobs in production: the **pgvector README**, where `m`, `ef_construction`, `hnsw.ef_search` are exactly the three prices above. And the bridge to the capstone: **Subramanya et al., "DiskANN" (NeurIPS 2019)** — what this graph becomes when it must live on an SSD instead of in RAM. T6.L3 prices that difference. Now go write `hnsw.rs`; the curve is waiting.
